defmodule OnCourse.Web.PromptQuestion.Controller do
  ...
  alias OnCourse.Quizzes.PromptQuestion
  ...
  def create(%Plug.Conn{} = conn, %{"prompt_question" => params, "topic_id" => topic_id}) do
    ...
    if Permission.can?(conn.assigns.current_user, :create, {topic, OnCourse.Quizzes.PromptQuestion}) do
      case Quizzes.add_prompt_question(topic, params) do
        {:ok, %PromptQuestion{}} ->
          conn
    ...
    cond do
      ...
      Permission.can?(conn.assigns.current_user, :delete, {pq.topic, PromptQuestion}) ->
        case Quizzes.delete(pq) do
          {:ok, %PromptQuestion{}} ->
            conn
            |> put_flash(:success, "Prompt Question deleted!")
            |> redirect(to: Path.topic_path(Endpoint, :show, pq.topic.id))
          {:error, cs} ->
            conn
            |> put_flash(:error, "Couldn't delete prompt question: #{inspect Ectoplasm.errors_on(cs)}")
            |> redirect(to: Path.topic_path(Endpoint, :show, pq.topic.id))
        end
